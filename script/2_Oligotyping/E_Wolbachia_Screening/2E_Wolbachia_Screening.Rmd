...
cd $PATH_MAIN/output/tsv

names=$(cut -f1  2E_MATCH_fragments.tsv | sed 1d)

i=0
for frag in $(cut -f2  2E_MATCH_fragments.tsv | sed 1d)
  do
    echo $frag
    anvi-script-get-primer-matches --samples-txt samples_wolbachia_screening.txt  \
      --primer-sequences $frag \
      --min-remainder-length 10 \
      --output-dir $PATH_MAIN/output/screening/${names[i]}
    i=i+1
  done
```
