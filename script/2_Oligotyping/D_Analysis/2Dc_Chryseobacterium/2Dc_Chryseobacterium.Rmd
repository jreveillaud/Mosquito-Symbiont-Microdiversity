---
title: "Chryseobacterium (oligotyping)"
...
path_chryseo <- "../../../../output/2_Oligotyping/2A/Chryseobacterium/2A_oligotyping_Chryseobacterium_sequences-c3-s1-a0.0-A0-M10"

# load the matrix count table
matrix_count <- read.table(paste0(path_chryseo, "/MATRIX-COUNT.txt"), header = TRUE) %>% t()
...
fasta <- readDNAStringSet(paste0(path_chryseo, "/OLIGO-REPRESENTATIVES.fasta"))
...
ref_oligo_med2 <- read.table("../../../../output/2_Oligotyping/2B/2B_REF_info_chryseobacterium.tsv", sep="\t", header = TRUE)

## select only the 3 oligotypes of Chryseobacterium
...
#### Edit new metadata with Percent_chryseobacterium
```{r}
...
# select Chryseobacterium
ps.chryseobacterium <- ps.filter %>% subset_taxa(Genus=="Chryseobacterium")

# add read depth of Chryseobacterium
sample_data(ps.filter)$Read_chryseobacterium <- sample_sums(ps.chryseobacterium)
sample_data(ps.filter) %>% colnames()
sample_data(ps.chryseobacterium) %>% colnames()

# add percent of Chryseobacterium
sample_data(ps.filter)$Percent_chryseobacterium <- sample_data(ps.filter)$Read_chryseobacterium / sample_data(ps.filter)$Read_depth

# round the percent of Chryseobacterium at 2 decimals
sample_data(ps.filter)$Percent_chryseobacterium <- sample_data(ps.filter)$Percent_chryseobacterium %>% round(2)

# extract metadata table
test <- data.frame(sample_data(ps.filter))

# merge this metadata table with the other
new.metadata <- data.frame(sample_data(ps)) %>% merge(test %>% dplyr::select(c(Sample, Read_depth, Read_chryseobacterium, Percent_chryseobacterium)), by="Sample")
...
oligo_CGC <- ps %>% 
  ...
  subset_taxa(ref=="oligotype_CGC (11) | size:66891 / node_N1030 (11) | size:64053")

oligo_CGC <- prune_taxa(taxa_sums(oligo_CGC) >= 1, oligo_CGC)
oligo_CGC <- prune_samples(sample_sums(oligo_CGC) >= 1, oligo_CGC)
  
oligo_CGC %>% taxa_sums() -> sum_oligo_1
oligo_CGC@sam_data$Read_depth %>% sum() -> sum_total_1

sum_oligo_1 / sum_total_1 *100 # 4.39%
sum_oligo_1 / 6452623 *100 # 1.03%

data.frame("Sample"=oligo_CGC@sam_data$Sample, 
           "Read_tot" = oligo_CGC@sam_data$Read_depth,
           "Read_CGC"=oligo_CGC %>% sample_sums(),
           "Percent_CGC"=round((oligo_CGC %>% sample_sums() / oligo_CGC@sam_data$Read_depth *100),2)) -> df1
mean(round((oligo_CGC %>% sample_sums() / oligo_CGC@sam_data$Read_depth *100),2))
median(round((oligo_CGC %>% sample_sums() / oligo_CGC@sam_data$Read_depth *100),2))
min(round((oligo_CGC %>% sample_sums() / oligo_CGC@sam_data$Read_depth *100),2))
max(round((oligo_CGC %>% sample_sums() / oligo_CGC@sam_data$Read_depth *100),2))

# Oligotype TAC

oligo_TAC <- ps %>% 
  subset_samples(Organ=="Whole" & Strain!="Guadeloupe") %>%
  subset_taxa(ref=="oligotype_TAC (30) | size:12689 / node_N1035 (30) | size:12689")

oligo_TAC <- prune_taxa(taxa_sums(oligo_TAC) >= 1, oligo_TAC)
oligo_TAC <- prune_samples(sample_sums(oligo_TAC) >= 1, oligo_TAC)
  
oligo_TAC %>% taxa_sums() -> sum_oligo_2
oligo_TAC@sam_data$Read_depth %>% sum() -> sum_total_2

sum_oligo_2 / sum_total_2 *100 # 1.97%
sum_oligo_2 / 6452623 *100 # 0.19%

data.frame("Sample"=oligo_TAC@sam_data$Sample, 
           "Read_tot" = oligo_TAC@sam_data$Read_depth,
           "Read_TAC"=oligo_TAC %>% sample_sums(),
           "Percent_TAC"=round((oligo_TAC %>% sample_sums() / oligo_TAC@sam_data$Read_depth *100),2)) -> df2
mean(round((oligo_TAC %>% sample_sums() / oligo_TAC@sam_data$Read_depth *100),2))
median(round((oligo_TAC %>% sample_sums() / oligo_TAC@sam_data$Read_depth *100),2))
min(round((oligo_TAC %>% sample_sums() / oligo_TAC@sam_data$Read_depth *100),2))
max(round((oligo_TAC %>% sample_sums() / oligo_TAC@sam_data$Read_depth *100),2))

# Oligotype TAT

oligo_TAT <- ps %>% 
  subset_samples(Organ=="Whole" & Strain!="Guadeloupe") %>%
  subset_taxa(ref=="oligotype_TAT (35) | size:26519 / node_N1034 (35) | size:26519")

oligo_TAT <- prune_taxa(taxa_sums(oligo_TAT) >= 1, oligo_TAT)
oligo_TAT <- prune_samples(sample_sums(oligo_TAT) >= 1, oligo_TAT)
  
oligo_TAT %>% taxa_sums() -> sum_oligo_3 
oligo_TAT@sam_data$Read_depth %>% sum() -> sum_total_3 

sum_oligo_3 / sum_total_3 *100 # 3.7%
sum_oligo_3 / 6452623 *100 # 0.41%

data.frame("Sample"=oligo_TAT@sam_data$Sample, 
           "Read_tot" = oligo_TAT@sam_data$Read_depth,
           "Read_TAT"=oligo_TAT %>% sample_sums(),
           "Percent_TAT"=round((oligo_TAT %>% sample_sums() / oligo_TAT@sam_data$Read_depth *100),2)) -> df3
mean(round((oligo_TAT %>% sample_sums() / oligo_TAT@sam_data$Read_depth *100),2))
median(round((oligo_TAT %>% sample_sums() / oligo_TAT@sam_data$Read_depth *100),2))
min(round((oligo_TAT %>% sample_sums() / oligo_TAT@sam_data$Read_depth *100),2))
max(round((oligo_TAT %>% sample_sums() / oligo_TAT@sam_data$Read_depth *100),2))
```


#### Percent by oligotype (whole + ovary)
```{r}
# Oligotype CGC
...
sum_oligo_1 / sum_total_1 *100 # 4.37%
sum_oligo_1 / 6452623 *100 # 1.04%
...
sum_oligo_3 / sum_total_3 *100 # 3.6%
sum_oligo_3 / 6452623 *100 # 0.41%
```
...
test3 <- test %>% select(c(Sample, Species, Strain, Organ, Read_depth, Read_chryseobacterium)) %>% reshape2::melt(id.vars=c("Sample", "Species", "Strain", "Organ"), vars=c("Read_depth", "Read_chryseobacterium"))
...
new_names <- c( "oligotype_CGC (11) | size:66891 / N1030 (11) | size:64053.",
                "oligotype_TAT (35) | size:26519 / N1034 (35) | size:26519.",
                "oligotype_TAC (30) | size:12689 / N1035 (30) | size:12689.")

data_for_plot2$Name <- factor(data_for_plot2$Name, levels = new_names)

col_add <- brewer.pal(8, "Accent")

col <- c("oligotype_CGC (11) | size:66891 / N1030 (11) | size:64053."="#F899FF",
         "oligotype_TAT (35) | size:26519 / N1034 (35) | size:26519."="#FFB6ED",
         "oligotype_TAC (30) | size:12689 / N1035 (30) | size:12689."="#F5D0E5")
...
levels(data_for_plot3$Strain) <- c("Guadeloupe", "Lavar~(lab)")
...
```{r warning=FALSE}
tiff("../../../../output/2_Oligotyping/2D/2Dc_OLIGO_counts_chryseobacterium.tiff", units="in", width=20, height=18, res=300)
p_counts
dev.off()

tiff("../../../../output/2_Oligotyping/2D/2Dc_OLIGO_taxonomic_chryseobacterium_whole.tiff", units="in", width=16, height=12, res=300)
p2
dev.off()

tiff("../../../../output/2_Oligotyping/2D/2Dc_OLIGO_taxonomic_chryseobacterium_ovary.tiff", units="in", width=18, height=14, res=300)
p3
dev.off()

tiff("../../../../output/2_Oligotyping/2D/2Dc_OLIGO_taxonomic_chryseobacterium.tiff", units="in", width=18, height=16, res=300)
p_taxo
dev.off()

png("../../../../output/2_Oligotyping/2D/2Dc_OLIGO_counts_chryseobacterium_big.png", units="in", width=20, height=18, res=300)
p_counts
dev.off()

png("../../../../output/2_Oligotyping/2D/2Dc_OLIGO_counts_chryseobacterium_small.png", units="in", width=18, height=14, res=300)
p_counts
dev.off()

png("../../../../output/2_Oligotyping/2D/2Dc_OLIGO_taxonomic_chryseobacterium_whole.png", units="in", width=16, height=12, res=300)
p2
dev.off()

png("../../../../output/2_Oligotyping/2D/2Dc_OLIGO_taxonomic_chryseobacterium_ovary.png", units="in", width=18, height=14, res=300)
p3
dev.off()

png("../../../../output/2_Oligotyping/2D/2Dc_OLIGO_taxonomic_chryseobacterium_big.png", units="in", width=18, height=18, res=300)
p_taxo
dev.off()

png("../../../../output/2_Oligotyping/2D/2Dc_OLIGO_taxonomic_chryseobacterium_small.png", units="in", width=18, height=14, res=300)
p_taxo
dev.off()
```
...
img <- magick::image_read(paste0(path_chryseo, "/HTML-OUTPUT/entropy.png"))
...
tiff("../../../../output/2_Oligotyping/2D/2Dc_OLIGO_main_chryseobacterium.tiff", width=36, height=36, res=300, units="in")
t_plot
dev.off()

png("../../../../output/2_Oligotyping/2D/2Dc_OLIGO_main_chryseobacterium.png", width=36, height=36, res=300, units="in")
t_plot
dev.off()
```
