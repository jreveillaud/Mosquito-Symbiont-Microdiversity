...
test3 <- test %>% select(c(Sample, Species, Location, Organ, Read_depth, Read_legionella)) %>% reshape2::melt(id.vars=c("Sample", "Species", "Location", "Organ"), vars=c("Read_depth", "Read_legionella"))
...
p3 <- ggplot(data_for_plot3, aes(x = Sample, y = Relative_Abundance, fill = Name, species=Species, organ=Organ, location=Location))+ 
  ...
  theme(legend.title = element_text(size = 20), 
        ...
        axis.ticks.x=element_blank(),
        axis.text.y = element_text(size=18)) +
  facet_wrap(~Species+Location+Organ, scales = "free", ncol=3)+
  labs(x="Sample", y="Relative abundance", fill="Oligotype / MED node")
